 ...
 * - La respuesta se RECALCULA por derivación independiente: se parsea el
 *   enunciado/contexto/meta del ejercicio y se deriva la solución sin
 *   reutilizar la fórmula del generador.
 * - Invariantes comunes: correct ∈ options, opciones únicas, 2-4 opciones.
 *
...
function checkInvariants(engine: string, ex: Exercise): boolean {
  if (!ex.options.includes(ex.correct)) {
    fail(engine, ex.type, 'correct no está en options', ex);
    return false;
  }
  if (new Set(ex.options).size !== ex.options.length) {
    fail(engine, ex.type, 'opciones duplicadas', ex);
    return false;
  }
  if (ex.options.length < 2 || ex.options.length > 4) {
    fail(engine, ex.type, `número de opciones inválido (${ex.options.length})`, ex);
    return false;
  }
  return true;
}
...
const mathChecks: Record<string, (ex: Exercise) => string | null> = {
  percent(ex) {
...
function deriveSeries(type: string, terms: number[]): number | 'unknown' {
...
const konzChecks: Record<string, (ex: Exercise) => string | null> = {
  blockdiff(ex) {
...
const analyseChecks: Record<string, (ex: Exercise) => string | null> = {
  modloop(ex) {
...
interface Suite {
  engine: string;
  generators: Record<string, { fn: () => Exercise }>;
  check: (ex: Exercise) => string | null;
}

// OJO: los checks devuelven null en caso de ÉXITO — no usar `?? 'error'`
// sobre su resultado (brief §5.6: los tests también se verifican).
const lookup =
  (checks: Record<string, (ex: Exercise) => string | null>) =>
  (ex: Exercise): string | null => {
    const fn = checks[ex.type];
    return fn ? fn(ex) : `sin verificador para tipo ${ex.type}`;
  };

const suites: Suite[] = [
  {
    engine: 'mathematik',
    generators: MATHEMATIK_GENERATORS,
    check: lookup(mathChecks),
  },
  {
    engine: 'zahlenreihen',
    generators: ZAHLENREIHEN_GENERATORS,
    check: (ex) => {
      const terms = ex.meta?.terms as number[];
  ...
  {
    engine: 'konzentration',
    generators: KONZENTRATION_GENERATORS,
    check: lookup(konzChecks),
  },
  {
    engine: 'analyse',
    generators: ANALYSE_GENERATORS,
    check: lookup(analyseChecks),
  },
];
...
for (const suite of suites) {
  for (const [typeKey, gen] of Object.entries(suite.generators)) {
    ...
    for (let i = 0; i < N; i++) {
      const ex = gen.fn();
      if (!checkInvariants(suite.engine, ex)) continue;
      // los generadores tienen fallback a otro tipo en casos degenerados:
      // verificar según el tipo REAL devuelto
      const err = suite.check(ex);
